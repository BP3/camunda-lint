    ...
    if (Object.keys(finalDeps).length > 0) {
        if (installDeps) {
            ...
            try {
                ...
                throw new Error(`Failed to run 'npm install' in temporary plugin directory: ${error.stderr.toString()}`);
            }
        } else {
             throw new Error(
                `Custom rules require dependencies, but they are not installed. ` +
                `Please use the '--install-custom-deps' flag.`
            );
        }
    }
}
...
async function findFiles(pattern) {
  ...
  const files = await tinyglob(pattern, { absolute: true, filesOnly: true, dot: true });
